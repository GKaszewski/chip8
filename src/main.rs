...
fn initialize_memory (chip8: &mut Chip8) {
            ...
            match kk {
                0x9E => {
                    ...
                    if chip8.keypad[chip8.v[second_nibble as usize] as usize] != 0 {
                        println!("Key pressed: {}", chip8.v[second_nibble as usize] as usize);
                        chip8.pc += 2;
                    }
                },
                0xA1 => {
                    ...
                    if chip8.keypad[chip8.v[second_nibble as usize] as usize] == 0 {
                        println!("Key not pressed: {}", chip8.v[second_nibble as usize] as usize);
                        chip8.pc += 2;
                    }
                },
                _ => println!("Unknown opcode: {:X}", opcode),
...
fn handle_keypads (chip8: & mut Chip8, rl_context: &RaylibHandle) {
        ...
        for y in 0..32 {
            for x in 0..64 {
                if chip8.display[(y * 64) + x] == 1 {
                    d.draw_rectangle((x * PIXEL_SIZE).try_into().unwrap(), (y * PIXEL_SIZE).try_into().unwrap(), PIXEL_SIZE as i32, PIXEL_SIZE as i32, Color::WHITE);
                }
            }
        }
